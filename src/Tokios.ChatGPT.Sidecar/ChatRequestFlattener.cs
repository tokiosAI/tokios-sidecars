...
public static class ChatRequestFlattener
{
    public static FlattenedRequest Flatten(JsonElement root)
    {
        ...
        foreach (var name in new[] { "tools", "tool_choice", "functions", "function_call" })
            if (root.TryGetProperty(name, out _))
                throw new ChatRequestException(
                    $"'{name}' is not supported: the Codex CLI runs its own tools. Send plain chat messages.");
        ...
        // OpenAI reasoning_effort ("minimal"|"low"|"medium"|"high") → codex turn effort levels.
        // The CLI's extra level (xhigh) is accepted verbatim; anything else is rejected
        ...
        if (root.TryGetProperty("reasoning_effort", out var re) && re.ValueKind == JsonValueKind.String)
        {
            effort = re.GetString()?.ToLowerInvariant() switch
            {
                ...
                var other => throw new ChatRequestException(
                    $"Unsupported reasoning_effort '{other}'. Use minimal|low|medium|high (or xhigh)."),
            };
        }
        ...
        // sees the conversation as text, since the app-server turn takes one flat input array.
